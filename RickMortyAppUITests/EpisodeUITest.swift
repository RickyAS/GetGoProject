...
final class EpisodeUITest: XCTestCase {
    ...
    func testScrollTableToLastPage() throws {
        start()
    ...
    func testSuccessFilterBySearch() throws {
        start()
        app.navigationBars["Episode"].searchFields.element.tap()
        let searchArr = Array("Total").map({String($0)})
        searchArr.forEach({
            app.keys[$0].tap()
        })
    ...
    func testFailFilterBySearch() throws {
        start()
        app.navigationBars["Episode"].searchFields.element.tap()
        let searchArr = Array("Average").map({String($0)})
        searchArr.forEach({
            app.keys[$0].tap()
        })
        app/*@START_MENU_TOKEN@*/.buttons["Search"]/*[[".keyboards",".buttons[\"search\"]",".buttons[\"Search\"]"],[[[-1,2],[-1,1],[-1,0,1]],[[-1,2],[-1,1]]],[0]]@END_MENU_TOKEN@*/.tap()
    ...
    private func start() {
        app.launch()
        app.tabBars["Tab Bar"].buttons["Episode"].tap()
    }
